...
static void b64_encode(char* dst, const unsigned char* src, int len, int out_chars) {
    ...
    // write out_chars characters, from most significant 6-bit groups
...
static void eks_blowfish_setup(BlowfishCtx& ctx,
    ...
    // Repeat ExpandKey with salt then key, 2^cost times
...
static std::string bcrypt_hashpw(const std::string& password,
    ...
    // Ciphertext: put L and R into byte array (8 bytes), pad to 23 bytes
