...
- ✅ 雪花 ID 生成（分布式友好）
- ✅ 邮件持久化队列（异步处理）
- ✅ 出站 SMTP Client 设计（Outbox + 重试 + 租约抢占）
...
│  └──────────────┘  └──────────────┘  └──────────────┘    │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐    │
│  │ Outbound调度 │  │ DeliveryWorker│  │ Outbox仓储    │    │
│  └──────────────┘  └──────────────┘  └──────────────┘    │
└─────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────┐
...
│   ├── smtp-outbound-client-design.md # SMTP 出站投递设计表
...
### 4.10 SMTP 出站投递模块 (Outbound Delivery)

#### 设计目标

在不影响现有 SMTPS 入站接收流程的前提下，引入独立的 SMTP Client 模块，负责将系统内邮件可靠投递到外部 SMTP 服务器。

#### 模块边界

- **SmtpsServer（入站）**: 接收外部来信并落地
- **PersistentQueue（入库）**: 持久化邮件元数据和附件信息
- **OutboundOrchestrator（出站调度）**: 拉取可投递任务并分发给 worker
- **SmtpDeliveryWorker（出站执行）**: 与外部 SMTP 建连、投递、处理响应
- **OutboxRepository（状态管理）**: 原子 claim 与状态更新
- **DeliveryDbExecutor（状态落库）**: 在任务线程池执行耗时 DB 更新
...
- 临时失败: `SENDING -> RETRY -> SENDING`
- 永久失败: `SENDING -> DEAD`

#### 一致性策略

- 使用 `lease_owner + lease_until` 防止多节点重复消费
- 支持 worker 宕机后的过期任务回收
- 采用“至少一次”投递语义

#### 线程模型（v1）

- 单 `Client` 主线程负责轮询、会话调度和完成队列收敛
- Asio IO 线程池负责 SMTP 网络 I/O
- 任务线程池负责 DB 状态更新（成功/重试/死信）
- 主线程不直接执行耗时 DB 操作，避免被阻塞

详细设计表见: `docs/smtp-outbound-client-design.md`
...
系统内生成待发送邮件
  ↓
写入 Outbox (status = PENDING)
  ↓
notifyNewOutboxItem(mail_ptr, outbox_id) 触发快速唤醒
  ↓
OutboundOrchestrator::claimBatch()
  ↓
状态: PENDING/RETRY -> SENDING（写入 lease）
  ↓
SmtpDeliveryWorker 执行 SMTP 投递
  ↓
将结果写入 CompletionQueue（线程安全）
  ↓
Client 主线程轮询 CompletionQueue
  ↓
投递到任务线程池执行 DB 状态更新:
  |- 成功: SENDING -> SENT
  |- 临时失败: SENDING -> RETRY (next_attempt_at)
  `- 永久失败: SENDING -> DEAD
  ↓
定时兜底轮询 + 过期租约回收
```
